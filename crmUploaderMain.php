...
//  error_reporting(E_ALL);
  ...
  require_once 'crmUploaderFunctions.php';
    ...
    $cacertPemFilePath = 'cacert.pem';  // only required on localhost (SSL)
    ...
    // get owner ID
    $ownerID = lookupOwnerID($ownerEmail, $hapikey, $cacertPemFilePath);
    ...
    // loop through all rows in the spreadsheet
        ...
        if(isset($dictionary[$email])){
            ...
            $contactIDAndURL = getContactIDFromAPI($email, $hapikey, $cacertPemFilePath);  // if no: ping the API for the VID
            // 2nd check: is there a contact with this VID?
            if($contactIDAndURL[0] == -1){  // 'no' case
              createNewContact($email, $firstname, $lastname, $hapikey, $ownerID, $cacertPemFilePath);  // create new contact in the CRM
              echo 'New contact created & owner assigned, ';
              $newContactsCounter++;  // increment counter of new contacts created
              $contactIDAndURL = getContactIDFromAPI($email, $hapikey, $cacertPemFilePath);  // ping the API for the VID again
            }else{
        ...
        $data = setDataSet($contactID, $ownerID, $type, $description, $date, $time, $timezone);
        // ping API to create engagement in HS CRM
        $statusCode = sendDataToEndpoint($hapikey, json_encode($data), $cacertPemFilePath);
