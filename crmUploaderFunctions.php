 ...
 * @param (string): CA Cert File path (only needed if used on localhost)
 * @return (array): contact ID (vid) and public contact URL (if contact exists) or -1 flag as first element (if contact doesn't exist)
 */
function getContactRecordID($cacertPemFilePath, $endpoint){
  $ch = curl_init();
  @curl_setopt($ch, CURLOPT_CAINFO, $cacertPemFilePath);	// this is needed to use SSL
 ...
 */
function getContactIDFromAPI($contactEmail, $apiKey, $cacertPemFilePath){
  $endpoint = setLookupByEmailEndpointUrl($contactEmail, $apiKey);
  return getContactRecordID($cacertPemFilePath, $endpoint);
}
 ...
 */
function sendDataToEndpoint($hapikey, $data, $cacertPemFilePath){
 ...
 */
function setDataSet($contactID, $ownerID, $type, $description, $date, $time, $timezone){
 ...
 * @param (string): CA Cert File path (only needed if used on localhost)
 */
function createNewContact($email, $firstname, $lastname, $hapikey, $ownerID, $cacertPemFilePath){
  $data = array(
         'properties' => array(
             array(
                 'property' => 'email',
     ...
     $ch = @curl_init();
     @curl_setopt($ch, CURLOPT_CAINFO, $cacertPemFilePath);	// this is needed to use SSL on localhost
     @curl_setopt($ch, CURLOPT_POST, true);
 ...
 */
function lookupOwnerID($ownerEmail, $hapikey, $cacertPemFilePath){
